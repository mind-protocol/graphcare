...
- **Content modification** -- All catalysts operate on topology: drives, structural nodes, energy values. Content and synthesis fields are never read or written. This is inherited from the topology-only principle.
...
- Auto-prescribed catalysts match the expected prescriptions for the citizen's brain category (testable against the prescription logic)
